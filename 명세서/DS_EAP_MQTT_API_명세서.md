...
Ver 3.4 · 2026-04-12 · 대외비  
Carsem 실가동 로그 14일 분석 반영 | v3.2(재혁) + v3.0(팀원) 통합 최종본

| 문서 번호 | DS-EAP-MQTT-API-001 v3.4 |
| :--- | :--- |
...
> [실측] **모바일 PASS 드롭 필터 의무 (Rule R39):** overall_result=PASS AND fail_count=0인 경우 inspection_detail / detail 그룹 파싱 스킵 필수. PASS 페이로드 약 2.1KB, takt 1,620ms 환경에서 전 슬롯 PASS detail 파싱은 불필요한 CPU·메모리 낭비. summary 그룹(overall_result, fail_count)만 파싱하여 모바일 수신 처리.
...
| ≥ 24,000s | LOT_END 미수신 의심 | CRITICAL — VISION_SCORE_ERR 알람 발행 (hw_error_detail: "LotController.StartNewLot") |
...
| burst_id | string (UUID) | 8d9e1f2a-... | N | 알람 폭주 그룹 ID. 동일 원인 연속 알람에 공통 UUID 부여. 단독 발생 시 null |
| burst_count | integer | 41 | N | burst_id 그룹 내 누적 알람 횟수. 단독이면 1. AggregateException 41건 연속 케이스 대응 |
...
| ALARM_ACK | MES / 모바일 | 알람 확인 처리. EAP가 `ds/{eq}/alarm` 토픽에 빈 페이로드+Retain=true 발행하여 retained 메시지 clear. `target_burst_id` 그룹 단위 ACK 가능 | Y (확인) |
...
| R24 | lot_duration_sec | <24,000s | - | ≥24,000s → VISION_SCORE_ERR 발행 | 09,10,16 |
| R25 | LOT Start/End 차이 | 0 | 1~3 누적 | ≥5 누적 | 16 |
| R26 | CAM_TIMEOUT_ERR/일 | 0건 | 1~2건 | >3건 | 11 |
| R27 | WRITE_FAIL 발생 | 0건 | 1건 이상 | 연속 5건 이상 | 12 |
| R28 | VISION_SCORE_ERR (NULL케이스, #4056) | 0건 | 산발 | 연속 10건 이상 | 13 |
...
| 재연결 백오프 | 지수 백오프 | 동일 | 동일 | 동일 | 1s→2s→5s→15s→30s, max 60s, jitter ±20% |
...
### A.5 Will 메시지 우선순위 처리 규칙 (모바일 앱 상태 머신)

모바일 앱은 EAP_DISCONNECTED Will 메시지 수신 시 아래 규칙에 따라 Heartbeat 타이머와 무관하게 즉시 처리해야 합니다.

| 우선순위 | 조건 | 동작 |
| :--- | :--- | :--- |
| 1 (최우선) | Will 메시지(EAP_DISCONNECTED) 수신 | 즉시 STOP 상태로 전환. Heartbeat 타이머 리셋 및 무시 |
| 2 | Heartbeat 9s 이상 미수신 | WARNING 배지 전환. Will 미수신 상태에서만 적용 |
| 3 | Heartbeat 30s 초과 미수신 | OFFLINE 배지 전환 |

> 주의: Will 메시지와 Heartbeat 9s OFFLINE 판단이 경합하는 경우 Will을 항상 우선 처리. Heartbeat 정상 수신 중 Will 도착 케이스(크래시 직후 잠시 수신 가능)도 즉시 STOP 처리 필수.

### A.6 재연결 백오프 규격

EAP 클라이언트가 Broker 연결 끊김 후 재연결 시도 시 적용하는 지수 백오프 수치입니다.

| 재시도 횟수 | 대기 시간 (jitter ±20% 적용) |
| :--- | :--- |
| 1회 | 1s |
| 2회 | 2s |
| 3회 | 5s |
| 4회 | 15s |
| 5회 이상 | 30s (최대 60s, jitter ±20%) |

> jitter 계산: 기준 대기시간 × (0.8 ~ 1.2) 범위 무작위 선택. 다수 장비 동시 재연결 시 Broker 과부하 방지 목적.
...
| 21_control_emergency_stop | CONTROL_CMD | §8 | EMERGENCY_STOP / MOBILE_APP | VISION_SCORE_ERR CRITICAL 후 긴급 정지 |
| 22_control_status_query | CONTROL_CMD | §8 | STATUS_QUERY / MOBILE_APP | 알람 후 즉시 상태 확인 |
| 23_oracle_normal | ORACLE_ANALYSIS | §9 | NORMAL / Carsem_3X3 / 96.2% / 28 LOT | 정상 2차 검증 결과 |
| 24_oracle_warning | ORACLE_ANALYSIS | §9 | WARNING / Carsem_4X6 / 68.5% / 8 LOT | WARNING 2차 검증 + 임계값 제안 |
| 25_oracle_danger | ORACLE_ANALYSIS | §9 | DANGER / Carsem_4X6 / 58.3% / 12 LOT | DANGER 3σ 이탈 + 즉시 중단 권고 |
...
| v3.4 | 2026-04-12 | Retained Message 정책 신설 (§1.1.1): status/lot/alarm/recipe/oracle 토픽 retained 의무화. Will 메시지 WillRetain=true 명시. STATUS_UPDATE 진행률 3필드 추가 (§3.1: current_unit_count / expected_total_units / current_yield_pct). Mobile Subscriber 세션 정책 부록 A.7 신설 (clean_start=false, session_expiry=1h, 재연결 시퀀스). CONTROL_CMD에 ALARM_ACK 명령 추가 (§8.2). §6.6 알람 ACK 및 Retained Clear 시퀀스 신설. Mock 26/27 신규 생성. |